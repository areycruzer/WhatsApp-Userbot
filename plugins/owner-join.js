...
let handler = async (m, { conn, text, usedPrefix, command, args, participants, isOwner }) => {
 ...
 if (e.length) await conn.reply(res, `*🎯 Hello Guys 𝙻𝙴𝙶𝙴𝙽𝙳-𝙿𝚄𝚂𝚄🌹 Bot Here!*

@918695072248 He Is My Creator If You Have Any Doubt
I Was Invited By *${m.name}*`, m, {
    mentions: d
     ...
     if (!e.length) await conn.reply(global.owner[1]+'@s.whatsapp.net', `≡ *INVITATION OF A GROUP*\n\n@${m.sender.split('@')[0]} has invited *${conn.user.name}* to group\n\n*${await conn.getName(res)}*\n\n*ID* : ${res}\n\n📌 link : ${args[0]}\n\nThe Bot Will Exit Automatically After\n\n ${msToDate(global.db.data.chats[res].expired - now)}`, null, {mentions: [m.sender]})
