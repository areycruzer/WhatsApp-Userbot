...
const handler = async (m, {command, conn}) => {
  ...
  if (command == 'bts') {
    const res = (await axios.get(`https://raw.githubusercontent.com/legendaditya03/WhatsApp-Userbot/main/src/Abhi-Json/random-bts`)).data;
    const res2 = await res[Math.floor(res.length * Math.random())];
    await conn.reply(m.chat, global.wait, m);
    conn.sendMessage(m.chat, {image: {url: res2}, caption: `_${command}_`.trim()}, {quoted: m});
  }
...
if (command == 'ff') {
    const res = (await axios.get(`https://raw.githubusercontent.com/legendaditya03/WhatsApp-Userbot/main/src/Abhi-Json/random-ff`)).data;
    const res2 = await res[Math.floor(res.length * Math.random())];
    await conn.reply(m.chat, global.wait, m);
    conn.sendMessage(m.chat, {image: {url: res2}, caption: `_${command}_`.trim()}, {quoted: m});
  }
