let handler = async (m, { conn, args, text, usedPrefix, command }) => {
	let tee = `🎯 Enter A Short Text\n\n📌Example  : *${usedPrefix + command}* LEGEND`
	let too = `🎯 Separate The Text With A *+* \n\n📌Example : \n*${usedPrefix + command}* LEGEND *+* PUSU`
	...
	switch (type) {
	  case 'hack':
		...
		conn.sendFile(m.chat, abhi, 'logo.png', `_Made By ©LEGEND-PUSU_`, m);
		m.react(done);
		break;
	  ...
	  case 'gfx2': 
		...
		conn.sendFile(m.chat, loda, 'logo.png', `_Made By ©LEGEND-PUSU_`, m);
